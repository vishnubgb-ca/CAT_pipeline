...
def preprocess():
    ...
    os.makedirs(join(os.getcwd(),npy_path, "imgs"), exist_ok=True)
    ...
    for name in tqdm(names[:]):
        ...
        gt_sitk = sitk.ReadImage(join(os.getcwd(),gt_path, gt_name))
        gt_data_ori = np.uint8(sitk.GetArrayFromImage(gt_sitk))


        '''
            PREPROCESSING STEP THAT WE ARE EXCLUDING AS IT IS NOT NECESSARY HERE
        ...
        if len(z_index) > 0:
            ...
            img_sitk = sitk.ReadImage(join(os.getcwd(),nii_path, image_name))
        ...
        np.savez_compressed(join(os.getcwd(),npy_path, prefix + gt_name.split(gt_name_suffix)[0]+'.npz'), imgs=img_roi, gts=gt_roi, spacing=img_sitk.GetSpacing())
        ...
        sitk.WriteImage(
            img_roi_sitk,
            join(os.getcwd(),npy_path, prefix + gt_name.split(gt_name_suffix)[0] + "_img.nii.gz"),
        )
        sitk.WriteImage(
            gt_roi_sitk,
            join(os.getcwd(),npy_path, prefix + gt_name.split(gt_name_suffix)[0] + "_gt.nii.gz"),
        )
